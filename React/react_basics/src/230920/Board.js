...
class Board extends Component {
    ...
    filterComment() {
        const result = this.state.selectCategory == 'writer' ? this.state.comments.filter((comment) => {
        ...
        return (
            <>
                <form>
                    ...
                    <input type="text" id="writer" value={this.state.inputWriter} onChange={(e) => this.onChange(e)} />
                    <label htmlFor="title">제목 :</label>
                    <input type="text" id="title" value={this.state.inputTitle} onChange={(e) => this.setState({ inputTitle: e.target.value })} />
                ...
                <div>
                    ...
                    <input placeholder="검색어" onChange={(e) => this.setState({ inputSearch: e.target.value })} />
                    <button type="button" onClick={this.filterComment}>검색</button>
                    <button type="button" onClick={this.showAll}>전체</button>
                </div>
                <table border={1} cellSpacing={0}>
                    ...
                    <tbody>
                        {this.state.onSearch ? this.state.filteredComments.map((value, index) => {
                            return <tr key={index}>
                                <td>{index + 1}</td>
                                <td>{value.title}</td>
                                <td>{value.writer}</td>
